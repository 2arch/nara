...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === "make") {
      ...
      if (paint) {
        let cells: Array<{ x: number; y: number; color: string }> = [];

        if (paint.cells) {
          cells = paint.cells;
        } else if (paint.rect) {
          const { x, y, width, height, color, filled = true } = paint.rect;
          cells = generateRectCells(x, y, width, height, filled).map(c => ({ ...c, color }));
        } else if (paint.circle) {
          const { x, y, radius, color, filled = true } = paint.circle;
          cells = generateCircleCells(x, y, radius, filled).map(c => ({ ...c, color }));
        } else if (paint.line) {
          const { x1, y1, x2, y2, color } = paint.line;
          cells = generateLineCells(x1, y1, x2, y2).map(c => ({ ...c, color }));
        }
      ...
      if (agent) {
        ...
        if (target && (move || action)) {
          ...
          if (target.id) {
            ...
            agentIds = (agentsResponse.result || []).map((a: any) => a.id);
          } else if (target.near) {
            ...
            agentIds = (agentsResponse.result || [])
              .filter((a: any) => a.spriteName === target.name)
              .map((a: any) => a.id);
          }
