...
function BdayPageContent() {
  const [cursorAlternate, setCursorAlternate] = useState(false);
  const [user, setUser] = useState<User | null>(null);
  const [authLoading, setAuthLoading] = useState(true);
  const [username, setUsername] = useState<string>('bday'); // Default to 'bday' for unauthenticated
  ...
  useEffect(() => {
    ...
    const visitPath = `worlds/public/bday/shares/visits/${Date.now()}`;
    const visitData = {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        setUsername('bday'); // Reset to default when logged out
      }
    });
