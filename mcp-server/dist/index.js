...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            case "move_agents": {
                const { agentIds, destination } = args;
            ...
            case "move_agents_expr": {
                const { agentIds, xExpr, yExpr, vars, duration } = args;
            ...
            // Agent-based command execution
            case "agent_command": {
                const { agentId, command, restoreCursor = true } = args;
            ...
            // Agent action with optional selection (atomic operation)
            case "agent_action": {
                const { agentId, command, selection } = args;
            ...
            case "sequence": {
                const { operations, delayMs = 50 } = args;
                const results = [];
            ...
            // Direct note creation
            case "create_note": {
                const { x, y, width, height, content } = args;
