...
exports.tools = exports.canvasTools = void 0;
exports.generateRectCells = generateRectCells;
exports.generateCircleCells = generateCircleCells;
exports.generateLineCells = generateLineCells;
...
exports.tools = exports.canvasTools;
// =============================================================================
// HELPER FUNCTIONS - Shape generation
// =============================================================================
function generateRectCells(x, y, width, height, filled) {
    const cells = [];
...
function generateCircleCells(centerX, centerY, radius, filled) {
    const cells = [];
...
function generateLineCells(x1, y1, x2, y2) {
    const cells = [];
...
function filterByLocation(entities, region, near) {
    let result = entities;
    if (region) {
        result = result.filter(e => e.x >= region.x && e.x < region.x + region.width &&
            ...
            case 'make': {
                ...
                if (args.paint) {
                    ...
                    if (rect) {
                        ...
                        const paintCells = generateRectCells(x, y, width, height, filled).map(c => (Object.assign(Object.assign({}, c), { color })));
                        ctx.paintCells(paintCells);
                        results.push(`Painted ${filled ? 'filled' : 'outlined'} rectangle`);
                    }
                    if (circle) {
                        const { x, y, radius, color, filled = true } = circle;
                        const paintCells = generateCircleCells(x, y, radius, filled).map(c => (Object.assign(Object.assign({}, c), { color })));
                        ctx.paintCells(paintCells);
                        results.push(`Painted ${filled ? 'filled' : 'outlined'} circle`);
                    }
                    if (line) {
                        const { x1, y1, x2, y2, color } = line;
                        const paintCells = generateLineCells(x1, y1, x2, y2).map(c => (Object.assign(Object.assign({}, c), { color })));
                        ctx.paintCells(paintCells);
                ...
                if (args.agent) {
                    ...
                    if (think && targetIds.length > 0 && ctx.agentThink) {
                        for (const agentId of targetIds) {
                            ...
                            if (thought) {
                                results.push(`Agent ${agentId} thinks: "${(_a = thought.thought) === null || _a === void 0 ? void 0 : _a.slice(0, 100)}..."`);
                ...
                if (args.run_script && ctx.runScript) {
                    ...
                    if (scriptResult.success) {
                        results.push(`Script executed: ${((_b = scriptResult.output) === null || _b === void 0 ? void 0 : _b.join(' | ')) || 'no output'}`);
                    }
                    else {
                ...
                if (args.edit_note && ctx.editNote) {
                    ...
                    switch (operation) {
                        case 'append':
                            if (!text)
                                return { success: false, error: 'append requires text' };
                            edit = { operation: 'append', text };
                            break;
                        case 'insert':
                            if (!text || !position)
                                return { success: false, error: 'insert requires text and position' };
                            edit = { operation: 'insert', text, position };
                            break;
                        case 'delete':
                            if (!range)
                                return { success: false, error: 'delete requires range' };
                            edit = { operation: 'delete', range };
                            break;
                        case 'replace':
                            if (!text || !range)
                                return { success: false, error: 'replace requires text and range' };
                        ...
                        case 'cell':
                            if (!cell)
                                return { success: false, error: 'cell operation requires cell object' };
